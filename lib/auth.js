  ...
  [kMakeProtectedResolverReplace] (policy, resolverFn, valueOverride) {
    return async (parent, args, context, info) => {
      ...
      if (!result) {
        parent = {
          ...parent,
          [info.fieldName]: typeof valueOverride === 'function' ? valueOverride(parent[info.fieldName]) : valueOverride
        }
      }
      return resolverFn(parent, args, context, info)
    }
            ...
            if (fieldName === '__typePolicy') {
              ...
              if (typeof schemaTypeField !== 'undefined') {
                if (typeof opts !== 'undefined' && !opts.enabled) {
                  if (!isNonNullType(schemaTypeField.type)) {
                    throw new MER_AUTH_ERR_FAILED_POLICY_CHECK('You can not do a replacement on a GraphQL scalar type that is not a String and is not optionally null')
                  }
                  const { valueOverride = '' } = opts
