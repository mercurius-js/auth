...
class Auth {
  ...
  [kGetAuthDirectiveAST](astNode) {
    if (Array.isArray(astNode.directives) && astNode.directives.length > 0) {
      const authDirective = astNode.directives.find(
        directive => directive.name.value === this[kAuthDirective]
      )
  ...
  [kMakeProtectedResolver](authDirectiveAST, resolverFn) {
    return async (parent, args, context, info) => {
      ...
      const result = await this[kApplyPolicy](
        authDirectiveAST,
        parent,
        args,
        context,
      ...
      if (typeof schemaType.getFields === 'function') {
        for (const [fieldName, field] of Object.entries(
          ...
          if (typeof field.astNode !== 'undefined') {
            ...
            const authDirectiveASTForField = this[kGetAuthDirectiveAST](
              field.astNode
            )
            if (authDirectiveASTForField !== null) {
              if (typeof field.resolve === 'function') {
                const originalFieldResolver = field.resolve
                field.resolve = this[kMakeProtectedResolver](
                  authDirectiveASTForField,
