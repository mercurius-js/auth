...
const { kApplyPolicy, kAuthContext, kAuthDirective, kAuthErrors, kApplyPolicyToSelections, kSetForbiddenFieldsToNull, kBuildCursor, kGetAuthDirectiveAST } = require('./symbols')
...
class Auth {
  constructor ({ applyPolicy, authContext }) {
    ...
    this[kAuthDirective] = new GraphQLDirective({ name: 'auth', locations: [] })
  }

  [kGetAuthDirectiveAST] (field) {
    if (field.astNode && field.astNode.directives && field.astNode.directives.length > 0) {
      const authDirective = field.astNode.directives.find(directive => directive.name.value === this[kAuthDirective].name)
  ...
  async [kApplyPolicyToSelections] (fieldTypeMap, parentDefinition, definition, context, cursor) {
    ...
    await Promise.all(definition.selectionSet.selections.map(async selection => {
      ...
      const authDirectiveAST = this[kGetAuthDirectiveAST](field)

      // If protected field, apply policy and apply errors
      if (authDirectiveAST !== null && !await this[kApplyPolicy](authDirectiveAST, context)) {
