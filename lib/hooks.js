...
class Policy {
  ...
  async walkQueryAST (fieldTypeMap, definition, context, cursor) {
    ...
    const errors = []
    const promises = definition.selectionSet.selections.map(async selection => {
      ...
      if (authDirectiveAST !== null) {
        ...
        if (canAccessField) {
          ...
          if (typeof selection !== 'undefined' && typeof selection.selectionSet !== 'undefined' && selection.selectionSet.selections.length > 0) {
            const nestedErrors = await this.walkQueryAST(namedType.getFields(), selection, context, newCursor)
