...
function filterTypeInterfaces (allowedTypes, typeConfig) {
  ...
  for (const typeInterface of typeConfig.interfaces) {
    // Find the allowed interface in the registered types
    const allowedInterface = allowedTypes.find((allowedType) => allowedType.name === typeInterface.name)

    // The interface should always be available
    interfaces.push(allowedInterface)
  }
  return interfaces
}
...
function buildType (allowedTypes, typeToBuild) {
  ...
  // Otherwise, return the registered type
  return allowedTypes.find((type) => type.name === typeToBuild.name)
}
    ...
    return {
      ...
      types: () =>
        allowedTypes.filter(
          (registeredType) =>
            // Find associated registered union type
            typeConfig.types.some((unionType) => unionType.name === registeredType.name) &&
            // Omit if not allowed
            filterDirectiveMap[registeredType.name] !== false
        )
    }
  }
...
function pruneSchema (schema, originalFilterDirectiveMap) {
  ...
  for (const type of schemaTypes) {
    ...
    if (isObjectType(type)) {
      allowedTypes.push(new GraphQLObjectType(buildTypeConfig(allowedTypes, filterDirectiveMap, type)))
    } else if (isInterfaceType(type)) {
      allowedTypes.push(new GraphQLInterfaceType(buildTypeConfig(allowedTypes, filterDirectiveMap, type)))
    } else if (isInputObjectType(type)) {
      allowedTypes.push(new GraphQLInputObjectType(buildTypeConfig(allowedTypes, filterDirectiveMap, type)))
    } else if (isUnionType(type)) {
      allowedTypes.push(new GraphQLUnionType(buildTypeConfig(allowedTypes, filterDirectiveMap, type)))
    } else {
  ...
  return new GraphQLSchema({
    ...schema.toConfig(),
    query: query && allowedTypes.find((type) => type.name === query.name),
    mutation: mutation && allowedTypes.find((type) => type.name === mutation.name),
